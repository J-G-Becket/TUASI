...
/**
 * TUASI Spider Search Engine Module
 *
 * PHP version 8.0.3
 *
...
if (!isset($skipSearch)){
	if (isset($_POST['searchtype'])) {
		...
		if ($newSearchType == "1"){
			...
			$resultQuery = mysqli_query($connTemp, "SELECT * FROM searches WHERE title LIKE '%$newQuery%' OR url LIKE '%$newQuery%' OR description LIKE '%$newQuery%' ORDER BY (rateup - ratedown) DESC, (ratedown + rateup) ASC, timeurl DESC LIMIT 500") or die(mysqli_error($connTemp));
			}
		else if ($newSearchType == "2"){
			$titleString = "Lowest Rated Results";
			$resultQuery = mysqli_query($connTemp, "SELECT * FROM searches WHERE title LIKE '%$newQuery%' OR url LIKE '%$newQuery%' OR description LIKE '%$newQuery%' ORDER BY (ratedown - rateup) DESC, (rateup + ratedown) ASC, timeurl DESC LIMIT 500") or die(mysqli_error($connTemp));
			}
		else if ($newSearchType == "3"){
			$titleString = "Latest Indexed Results";
			$resultQuery = mysqli_query($connTemp, "SELECT * FROM searches WHERE title LIKE '%$newQuery%' OR url LIKE '%$newQuery%' OR description LIKE '%$newQuery%' ORDER BY timeurl DESC LIMIT 500") or die(mysqli_error($connTemp));
			}
	...
	if (!empty(mysqli_fetch_array( $resultQuery ))){
		foreach($resultQuery as $resQuery){
			...
			echo '<form method="post" action="rateurl.php">';
