...
if (!isset($skipSearch)){
	if (isset($_POST['searchtype'])) {
		...
		if ($newSearchType == "1"){
			...
			$resultQuery = mysqli_query($connTemp, "SELECT * FROM searchesuser WHERE url LIKE '%$newQuery%' ORDER BY (rateup - ratedown) DESC, (ratedown + rateup) ASC, timeurl DESC LIMIT 500") or die(mysqli_error($connTemp));
			}
		else if ($newSearchType == "5"){
			$titleString = "Title Specific Results";
			$resultQuery = mysqli_query($connTemp, "SELECT * FROM searchesuser WHERE title LIKE '%$newQuery%' ORDER BY (rateup - ratedown) DESC, (ratedown + rateup) ASC, timeurl DESC LIMIT 500") or die(mysqli_error($connTemp));
			}
		else if ($newSearchType == "6"){
			$titleString = "Description Specific Results";
			$resultQuery = mysqli_query($connTemp, "SELECT * FROM searchesuser WHERE description LIKE '%$newQuery%' ORDER BY (rateup - ratedown) DESC, (ratedown + rateup) ASC, timeurl DESC LIMIT 500") or die(mysqli_error($connTemp));
			}
		else {
